...
# Introduction
This is a tutorial to accompany Lab 1. Below you will find code for running your analyses. Data for this tutorial are from your textbook's eResources (Kamden & Mwavita).
...
Then use `describe()` from the `psych` package is one of the more popular functions for descriptive statistics.
...
This can be assessed based on the study design (e.g., do participants cross groups?). In this study, schools do not share funding streams and we can assume that this assumption is tenable.

## Normality
Examine the skew and kurtosis values from our `describe()` output. Generally, 0 ± 2 is a good rule of thumb for a tenable assumption of normality.
...
### Histograms
```{r, echo = TRUE, eval = FALSE}
ggplot(data = df, aes(x = school_funding)) +
  geom_histogram()
```
...
ggplot(data = df, aes(x = group, y = school_funding)) +
  geom_boxplot()
```
...
head(df_wide)
```
Our new wide data frame has three columns, one for each group, with rows of values for the level of school funding.
...
3. Calculating the mean between and within squares, we need to divide the between group sum of squares (SSB) by the no. of groups - 1, and divide the within group sum of squares (SSB) by
the sample size - no. of groups.
```{r}
...
4. To obtain the F-ratio we divide the mean between group sum of squares (MSB) by the mean within group sum of squares (MSW)
```{r}
# calculating F-ratio
Fratio <- MSB/MSW
Fratio
...
5. To determine whether the F-ratio is statistically significant, we will use the `pf()` function, entering in the degrees of freedom between groups df (df = 2) and the within groups df (df = 635). `lower.tail = FAlSE` conduct a one-way test of significance.
```{r}
