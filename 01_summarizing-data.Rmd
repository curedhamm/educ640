...
# Introduction
This is a tutorial to accompany Lab 1. Below you will find code for running your analyses using data from your textbook's eResources (Strunk & Mwavita, 2020).
...
```
We can also look at the structure of the data frame using the `str()` command and the variables are correct.
```{r echo = FALSE, eval = TRUE}
str(df)
```
...
This can be assessed based on the study design (e.g., do participants cross groups?). For this exercise, we can assume that this assumption is tenable.
...
3. Calculating the mean between and within squares, we need to divide the between group sum of squares (SSB) by the no. of groups - 1, and divide the within group sum of squares (SSW) by
the sample size - no. of groups. You can refer back to your descriptive output to get the group and sample sizes needed.
```{r}
...
4. To obtain the _F_-ratio we divide the mean between group sum of squares (MSB) by the mean within group sum of squares (MSW)
```{r}
# calculating F-ratio
Fratio <- MSB/MSW
Fratio
...
5. To determine whether the _F_-ratio is statistically significant, we will use the `pf()` function, entering in the degrees of freedom between groups (_df_ = 2) and the within groups (_df_ = 635). `lower.tail = FAlSE` conduct a one-way test of significance.
```{r}
