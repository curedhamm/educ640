...
Doing within-subjects ANOVA in R requires restructuring the data in a format that's better to work with.
...
First, let's view what the data looks like. I'm only going to look at the top five rows here.
```{r}
head(df)
```

Notice that this dataset doesn't have an ID column. We need to add that in, and the `rowid_to_column()` function is an easy way to do that. We can call the column `student_id`. Note: if your dataset already has an ID column, you don't need to do this.
```{r}
df <- df %>% 
...
Next, we need to pivot the data. When data is represented this way, with multiple scores for each student, it is called being in a *wide* format. To run an ANOVA, the data needs to be in a *long* format -- basically, this means that all the scores will be in one column and what type of score it is (pre, post, or followup) will be indicated in another column.

Let's illustrate. Right now, the variable is the type of test + score (for examplee, `pre_gsi`). Since we are making a type of test variable, let's rename the variables so it makes more sense.
...
With corrections, our new alpha is .05/2 = .025

While the pre-test differs from the other tests by 5.15 points, the difference is not significant (p = .022). The partial eta squared of 0.34 indicates that test type explain 34% of the difference between the pre-test score and scores from the other two tests.
